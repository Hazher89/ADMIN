...
interface AbsenceRequest {
  ...
  endDate: string;
  reason: string;
  type: 'sick_leave' | 'personal_leave' | 'other';
...
export default function AbsencePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    endDate: '',
    reason: '',
    type: 'sick_leave' as 'sick_leave' | 'personal_leave' | 'other'
  });
  ...
  const loadData = async () => {
    try {
      ...
      const requestsSnapshot = await getDocs(collection(db, 'absenceRequests'));
  ...
  // Filter requests
  useEffect(() => {
    const filtered = absenceRequests.filter(request => {
  ...
  }, [absenceRequests, searchTerm, statusFilter, typeFilter]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'absenceRequests'), requestData);
  ...
  const handleDelete = async (requestId: string) => {
    if (!confirm('Er du sikker på at du vil slette denne fraværsforespørselen?')) return;

    try {
      if (!db) throw new Error('Firebase not initialized');
      await deleteDoc(doc(db, 'absenceRequests', requestId));
      
      setAbsenceRequests(prev => prev.filter(request => request.id !== requestId));
    } catch (error) {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'personal_leave': return 'badge-primary';
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#666' }}>Laster fraværsystem...</p>
      ...
      <div className="page-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p className="page-subtitle">Administrer fraværsforespørsler</p>
          </div>
          <button
                ...
                <tr key={request.id} style={{ borderBottom: '1px solid rgba(0, 0, 0, 0.05)' }}>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <span className={`badge ${getTypeColor(request.type)}`}>
                      {request.type === 'sick_leave' ? 'Sykefravær' : 
                       request.type === 'personal_leave' ? 'Personlig fravær' : 'Annet'}
                    </span>
                  </td>
                  <td style={{ padding: '1rem', fontSize: '0.875rem', color: '#333' }}>
                    {request.days} dager
          ...
          <div style={{ textAlign: 'center', padding: '3rem' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: '#666' }}>
              {searchTerm || statusFilter !== 'all' || typeFilter !== 'all'
                ...
                : 'Ingen fraværsforespørsler funnet. Legg til en ny forespørsel for å komme i gang.'}
            </p>
          </div>
        )}
      </div>
                ...
                <div style={{ display: 'grid', gap: '1rem', marginBottom: '1.5rem' }}>
                  ...
                  <div>
                    ...
                    <select
                      required
                      value={formData.type}
                      onChange={(e) => setFormData({...formData, type: e.target.value as 'sick_leave' | 'personal_leave' | 'other'})}
                      className="form-input"
                    >
                      <option value="sick_leave">Sykefravær</option>
                      <option value="personal_leave">Personlig fravær</option>
